...
def fetch_data(symbol, end_date, interval='1d'):
    ...
    try:
        ...
        if data.empty or len(data) < 100:
            ...
            return np.array([])
        logging.info(f"Fetched {interval} data for {symbol} from {start_date} to {end_date}, rows: {len(data)}")
...
def compute_indicators(data, indicator, period, source='Close'):
    ...
    if source not in source_idx:
        logging.warning(f"Source '{source}' invalid. Defaulting to 'Close'.")
        source = 'Close'
